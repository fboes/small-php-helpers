 ...
 * Extend this class for doing the real test. Methods with "test" prefixed get tested. Methods with 'data'
 * prefixed are used as data providers for corresponding "test"-methods. "data"-methods MUST return an array of arrays.
...
class Tester
{
    ...
    public function __construct()
    {
        ...
        $this->colors = (strpos(php_uname('s'), 'Windows') === false && !isset($_GET[self::PARAM_FORCE_CLI]));
    }
    ...
    public function doTests()
    {
        ...
        if ($this->cli) {
            ...
            echo(
                ...
                .'table {width:100%;} th,td {border-bottom:1px dotted #ddd;font-weight:normal;}'
                .'th {text-align:left;padding-right:1em;}'
                .'.success {color:green;font-weight:bold;text-align:right;}'
                .'.fail {color:maroon;font-weight:bold;text-align:right;} .result {text-align:right;color:#999;}'
        ...
        foreach ($this->methodsTests as $m) {
            ...
            foreach ($data as $run => $dataSet) {
                ...
                if ($this->cli) {
                    echo("\n".'Success / tests: '.(int)$this->assertionsSuccess.'/'
                        .(int)$this->assertionsDone.'; duration: '
                        .round($this->testEnd - $this->testStart).' ms'."\n");
                } else {
                    echo('  </table>'."\n");
                    echo('  <p class="result">Success / tests: '
                        .(int)$this->assertionsSuccess.'/'.(int)$this->assertionsDone.'; duration: '
                        .round($this->testEnd - $this->testStart).' ms</p>'."\n\n");
                    echo('</div>'."\n");
                }
        ...
        $this->assertTrue(
            $this->globalAssertionsSuccess == $this->globalAssertionsDone,
            'Expecting all tests to succeed'
        );
        if ($this->cli) {
            echo("\n".'Sum success / tests: '.(int)$this->globalAssertionsSuccess.'/'
                .(int)$this->globalAssertionsDone."\n");
        } else {
            echo('  </table>'."\n");
            echo('  <p class="result">Sum success / tests: '.(int)$this->globalAssertionsSuccess.'/'
                .(int)$this->globalAssertionsDone.'</p>'."\n");
    ...
    public function assertRegExp($regExp, $value, $message = 'Expecting %s to match regular expression %s')
    {
        if (!is_string($regExp)) {
            ...
            return $this->assertTrue(false, sprintf('Expecting %s to be a string', $this->literalize($value)));
        }
        return $this->assertTrue(
            (bool)preg_match($regExp, $value),
            sprintf($message, $this->literalize($value), $this->literalize($regExp))
        );
    }
    ...
    public function assertMethodExists($methodName, $className, $message = 'Expecting class %s to have method %s')
    {
        ...
        if (!class_exists($className)) {
            ...
            return $this->assertTrue(
                ...
                sprintf($message, $this->literalize($className), $this->literalize($methodName))
            );
        }
    }
    ...
    public function assertClassHasAttribute(
        $attributeName,
        $className,
        $message = 'Expecting class %s to have attribute %s'
    ) {
        ...
        if (!class_exists($className)) {
            ...
            return $this->assertTrue(
                property_exists($className, $attributeName),
                sprintf($message, $this->literalize($className), $this->literalize($attributeName))
            );
        }
    }
     ...
     * @param  string $xsdUri  [description]
     * @param  string $message [description]
     * @return bool            [description]
     * @throws \Exception
     */
    public function assertValidXml($xml, $xsdUri = null, $message = 'Expecting string to be valid XML')
    {
        ...
        if ($success && !empty($xsdUri)) {
            ...
            return $this->assertTrue(
                $success,
                sprintf('Expecting XML to validate against XSD %s', $this->literalize($xsdUri))
            );
        }
