 ...
 *
 * $string = SuperString::init('Test')->str_nice_shorten()->paragraph()->r();
 * $string = (string)SuperString::init('Test')->str_nice_shorten()->paragraph();
 * SuperString::init('Test')->str_nice_shorten()->paragraph()->e(); // echo
 *
 *
...
class SuperString
{
    ...
    public function textile($singleLine = false)
    {
        ...
        $str = preg_replace('/(\S+)\((.+?)\)/s', '<abbr title="$2">$1</abbr>', $str);
    ...
    public function addingTubes($autoplay = false)
    {
        $this->string = preg_replace(
            ...
            '<div class="video-player youtube"><iframe allowfullscreen="allowfullscreen"'
            .' src="https://www.youtube.com/embed/$2?enablejsapi=1'.($autoplay ? '&amp;autoplay=1' : '')
            .'"></iframe></div>',
            $this->string
        );
        $this->string = preg_replace(
            '/(<p>)\s*(?:<a.+>)?[^<]*?vimeo.com\/(\d+)[^>]*?(?:<\/a>)?\s*(<\/p>)/s',
            '<div class="video-player vimeo"><iframe allowfullscreen="allowfullscreen"'
            .' src="https://player.vimeo.com/video/$2'.($autoplay ? '?autoplay=1' : '').'"></iframe></div>',
            $this->string
        );
        return $this;
    }
     ...
     * Arguments will _not_ be quoted. Uses magic arguments.
     *
     * @see  _vsprintf()
     * @return  self
     */
     ...
     * @return  self
     */
    public function strShorten($maxChars = 72, $weight = 100, $replace = '…')
    {
        ...
        if ($strLen > $maxChars) {
            ...
            switch ($border) {
                ...
                case 0:
                    ...
                    break;
                default:
                    $this->string = mb_ereg_replace('^(.{'.$border.'}).+(.{'.($maxChars - $border -1).'})$', '\1'
                        . $replace . '\2', $this->string);
                    break;
            }
        }
        return $this;
        ...
        if (!empty($langCode)) {
            ...
            if ($withHyphenation) {
                switch ($langCode) {
                    case 'de':
                        $str = mb_ereg_replace(
                            '(\w)(lich|dorf|stadt|burg|berg|markt|straße|baden|tig|den)(\W)',
                            '\1­\2\3',
                            $str
                        );
