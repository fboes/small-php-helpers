...
class Conversion
{
     ...
     * @param  int $precision with negative values the current $meters will be rounded to
     *                        the nearest 10, 100, etc meters
     * @return string         [description]
     */
    public static function returnDistance($meters, $unit = 'm', $precision = 0)
    {
    ...
    /**
     * Convert bearing into plain text bearing description. Will use _() for translation afterwards.
     * Can also be used for relative bearing
     * @param  float $deg [description]
     * @param  array $directions [description]
     * @param  float $fullCircle full circle relative to $deg
     * @return  string [description]
     */
    public static function returnBearing($deg, $directions = array('North','East','South', 'West'), $fullCircle = 360.0)
    {
        $directionsCount = count($directions);
        $segmentSize = $fullCircle / $directionsCount;
        $key = (int)round($deg / $segmentSize);
