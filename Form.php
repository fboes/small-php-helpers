...
class Form
{
    ...
    /**
     * Initiate form
     * @param array $defaultValues Set default values for all form fields, e.g. text=test for
     *                             <input name="test" value="text" />
     */
     ...
     * @param array $defaultValues Set default values for all form fields, e.g. text=test for
     *                             <input name="test" value="text" />
     * @return \fboes\SmallPhpHelpers\Form
     */
        ...
        foreach ($elements as $fieldname => $attributes) {
            if (is_array($attributes)) {
                if (empty($this->defaultElementAttributes[$fieldname])) {
                    ...
                    $this->defaultElementAttributes[$fieldname] =
                        array_merge($attributes, $this->defaultElementAttributes[$fieldname]);
                }
            } else {
     ...
     * @param string $html with %1$s being the label, %2$s being the actual form field, %3$s being the optional
     *                     error message, %4$s being an optional hint
     * @return $this
     * @throws \Exception
     */
    ...
    {
        $element = new Element(self::HTML_FORM, $html);
    ...
    /**
     * Add input field. Some input types will spawn a data-pattern-attribute containing rules to validate
     * this field via JavaScript.
    ...
    {
        $element = new Element(self::HTML_INPUT, $html, $options);
        ...
        if (!empty($element->attributes['decimals'])) {
            switch ((int)$element->attributes['decimals']) {
                ...
                case 2:
                    ...
                    break;
                default:
                    $element->attributes['step'] = 10 / (pow(10, $element->attributes['decimals'] + 1));
                    break;
            }
        ...
        if (!Form::isBlank($element->attributes['capture'])) {
        ...
        if (!Form::isBlank($element->attributes['maxlength'])) {
            if (!Form::isBlank($element->attributes['value'])
                && mb_strlen($element->attributes['value']) > (int)$element->attributes['maxlength']
            ) {
                $element->addError(
                    'maxlength',
                    sprintf(
        ...
        if (!Form::isBlank($element->attributes['value'])) {
            if (!Form::isBlank($element->attributes['pattern'])
                || !Form::isBlank($element->attributes['data-pattern'])
            ) {
                $pattern = !Form::isBlank($element->attributes['pattern'])
                    ? $element->attributes['pattern']
                    : $element->attributes['data-pattern']
                ;
            ...
            if (!Form::isBlank($element->attributes['max'])
                && (float)$element->attributes['value'] > (float)$element->attributes['max']
            ) {
                    $element->addError(
                        'max',
                        sprintf(
                            _('Field value is to big, maximum is %s.'),
                            $element->attributes['max']
                        )
                    );
            }
            if (!Form::isBlank($element->attributes['min'])
                && (float)$element->attributes['value'] < (float)$element->attributes['min']
            ) {
                    $element->addError(
                        'min',
                        sprintf(
                            _('Field value is to small, minimum is %s.'),
                            $element->attributes['min']
                        )
                    );
            }
        }
        if (!Form::isBlank($element->attributes['pattern']) || !Form::isBlank($element->attributes['data-pattern'])) {
    ...
    /**
     * Create standalone datalist (e.g. if you want to use a datalist with multiple input fields).
     * If you want to use a datalist with only one input use $this->input() for convenience.
     * HTML has to contain id-attribut, reference datalist via list-attribute
     * @param  string $html    [description]
    ...
    {
        $element = new Element(self::HTML_INPUT_OPTIONS_WRAPPER, $html, $options);
    ...
    {
        $element = new Element(self::HTML_TEXTAREA, $html);
        $element->throwExceptionOnEmpty('name');
        $element->setAttributesByArray($this->getdefaultElementAttributes($element->attributes['name']));
        $element->addDefaultValue(
            (!empty($element->attributes[self::ATTRIBUTE_CONTENT]))
    ...
    {
        $element = new Element(self::HTML_SELECT, $html, $options);
        $element->throwExceptionOnEmpty('name');
        if (empty($options)) {
    ...
    {
        $element = new Element(self::HTML_CHECKBOXES, $html, $options);
        $element->throwExceptionOnEmpty('name');
        if (empty($options)) {
            throw new \Exception('No options given');
        }
    ...
    {
        $element = new Element(self::HTML_BUTTON, $html);
    ...
    {
        return $this->storeElement(new Element($html));
    }

    /**
     * Get default attribute array as given in setDefaultElementAttributes for the form field with
    ...
    public function getdefaultElementAttributes($name)
    {
        if (preg_match('#^(.+)(\[\])$#', $name, $nameParts)) {
            return (!self::isBlank($this->defaultElementAttributes[$nameParts[1]]))
                ? $this->defaultElementAttributes[$nameParts[1]]
                : array()
            ;
        } else {
            return (!self::isBlank($this->defaultElementAttributes[$name]))
                ? $this->defaultElementAttributes[$name]
                : array()
            ;
        }
    }
    ...
    public function getdefaultElementAttribute($name, $key = 'value')
    {
        ...
        return (!self::isBlank($attributes[$key]))
            ? $attributes[$key]
            : null
        ;
    }
     ...
     * @return Form       [description]
     */
    protected function storeElement(Element $element)
    {
    ...
    public function returnHTML()
    {
        ...
        foreach ($this->formElements as $id => $element) {
            $return .= $element->returnHtml(
                $this->htmlFieldWrapper,
                $this->htmlLabelWrapper,
                $this->htmlLabelRequired,
                $this->htmlErrorWrapper
            );
        }
        return $return;
    }
     ...
     */
    public static function isBlank(&$v)
    {
