...
class Audio extends Media
{
    ...
    protected function returnHtml5Tag(array $remainingMediaObjects)
    {
        ...
        if (!empty($remainingMediaObjects['audio/mp4'])
            || !empty($remainingMediaObjects['audio/ogg'])
            || !empty($remainingMediaObjects['audio/webm'])
            || !empty($remainingMediaObjects['audio/mpeg'])
        ) {
            $attributes = array(
                'controls="controls"',
            ...
            foreach (array('audio/mp4','audio/ogg','audio/webm','audio/mpeg') as $mimeType) {
                if (!empty($remainingMediaObjects[$mimeType])) {
                    $html .= "\t".'<source src="'
                        .htmlspecialchars($remainingMediaObjects[$mimeType]).'" type="'
