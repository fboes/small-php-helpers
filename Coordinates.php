...
class Coordinates
{
     ...
     */
    public static function onSpecialPlanet($equatorialRadius, $polarRadius)
    {
     ...
     * @return  self [description]
     */
    public static function set($latitude, $longitude, $altitude = 0.0, $title = null)
    {
        $obj = new static();
    ...
    /**
     * Get new coordinates relative to current coordinates by using given distance and initial bearing, ignoring
     * altitude.
     * @see http://www.movable-type.co.uk/scripts/latlong.html
     ...
     */
    public function getRegularPolygon($distance, $vertices = 4, $offsetDeg = 0.0)
    {
        $verticeCoords = array();
        $step = 360 / $vertices;
    ...
    /**
     * Check if current coordinates are in bounds. If latitude is out of bounds this function will also convert
     * longitude, implying the given coordinates are on a line extending beyond the pole.
     * @return self [description]
     */
     ...
     * @param  string $format see sprintf() for options. The first parameter is decimal degrees, the second parameter
    ...
    public static function returnCoordinate($value, $plusPrefix, $minusPrefix, $format = '%1$09.5f %2$s')
    {
        return sprintf($format, abs($value), static::convertPrefix($value, $plusPrefix, $minusPrefix));
    }

    protected static function convertPrefix($value, $plusPrefix, $minusPrefix)
    {
        return ($value < 0) ? $minusPrefix : $plusPrefix;
    }
}
